...
The orchestrator supports several LLM backends, selectable at deployment without changing agent logic. In the current implementation, we rely on the Gemini API for reliable multi-step function calling at the cost of external data transmission, and on Ollama for local inference with no external transmission. The tool layer consists of six specialized MCP servers, plus one fhir-mcp server that provides patient record retrieval via FHIR over HTTP. Each server run as an independent process in its own virtual environment to prevent dependency conflicts %(most critically the GPU-enabled PyTorch required by \texttt{mcp-monai} versus the CPU-only install used elsewhere). 
Tool names are auto-prefixed by server (e.g.\ \texttt{monai.run\_inference}), providing unambiguous namespacing. The six core servers are: \texttt{mcp-monai} (medical image segmentation and detection), \texttt{mcp-radlex} (RadLex terminology and report templates), \texttt{mcp-utils} (DICOM parsing and filesystem operations), \texttt{mcp-skills} (skill workflow loading and execution), \texttt{mcp-ipath} ( telepathology integration), and \texttt{mcp-cellpose} (2D/3D cell segmentation). 
...
\textbf{Mcp-utils} provides DICOM metadata extraction (per file or directory) and general filesystem operations (read, write, move, copy, delete, list, search). 
...
\textbf{Mcp-skills} exposes the skill system as callable tools for loading a skill's entry point, references, and predefined scripts.
