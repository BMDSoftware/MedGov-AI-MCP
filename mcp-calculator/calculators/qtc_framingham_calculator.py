...
@register_calculator("qtc_framingham")
class QTcFraminghamCalculator(BaseCalculator):
    """QTc Framingham计算器实现"""
    ...
    def get_info(self) -> CalculatorInfo:
        return CalculatorInfo(
            ...
            category="cardiology",
            description="Calculate corrected QT interval using Framingham formula",
            parameters=[
                Parameter(
                    name="qt_interval",
    ...
    def calculate(self, parameters: Dict[str, Any]) -> CalculationResult:
        ...
        # 计算QTc: QTc = QT间期 + 154 * (1 - RR间期)
        qtc = round_number(qt_interval + (154 * (1 - rr_interval_sec)))
        ...
        return CalculationResult(
            ...
            metadata={
                ...
                "formula": "QTc = QT + 154 * (1 - RR interval)"
            }
        )
    ...
    def _generate_explanation(self, qt_interval: float, heart_rate: float, 
        ...
        """生成计算解释"""
        explanation = "The corrected QT interval using the Framingham formula is computed as QTc = QT Interval + (154 * (1 - rr_interval_sec)), "
        ...
        explanation += f"Hence, plugging in these values, we will get {qt_interval} + (154 * (1 - {rr_interval_sec})) = {qtc}.\n"
