"""
Albumin Corrected Delta Gap Calculator
"""
...
@register_calculator("albumin_corrected_delta_gap")
class AlbuminCorrectedDeltaGapCalculator(BaseCalculator):
    """白蛋白校正Delta Gap计算器实现"""
    ...
    def get_info(self) -> CalculatorInfo:
        return CalculatorInfo(
            ...
            category="laboratory",
            description="Calculate albumin-corrected delta gap",
            parameters=[
                Parameter(
                    name="sodium",
    ...
    def validate_parameters(self, parameters: Dict[str, Any]) -> ValidationResult:
        ...
        # 验证碳酸氢盐范围
        if not (1 <= bicarbonate <= 40):
            errors.append("Bicarbonate must be between 1 and 40 mEq/L")
    ...
    def calculate(self, parameters: Dict[str, Any]) -> CalculationResult:
        ...
        return CalculationResult(
            ...
            metadata={
                ...
                "formula": "Corrected Delta Gap = Corrected AG - 12"
            }
        )
    ...
    def _generate_explanation(self, sodium: float, chloride: float, bicarbonate: float, 
                            ...
                            corrected_delta_gap: float) -> str:
        """生成计算解释"""
        explanation = "To compute the formula of albumin corrected delta gap, the formula is albumin corrected anion gap (in mEq/L) - 12.\n\n"
        
        explanation += "First, we calculate the albumin corrected anion gap:\n"
        ...
        explanation += f"Plugging in {corrected_anion_gap} mEq/L for the anion gap into the albumin corrected delta gap formula, we get {corrected_anion_gap} - 12 = {corrected_delta_gap} mEq/L.\n"
        explanation += f"Hence, the patient's albumin corrected delta gap is {corrected_delta_gap} mEq/L."
