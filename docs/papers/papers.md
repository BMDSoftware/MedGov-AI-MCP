# Papers

`[x]` = read  `[ ]` = not yet read

---

## Conference Papers

Conferences to search for:

For medical ones
- AMIA
- IEEE EMBC
- IEEE ISBI
- CHIL

AI ones

- AAAI
- IJCAI
- ICLR
- ACL

### MICCAI 2025

- `[ ]` [WSI-Agents: A Collaborative Multi-Agent System for Multi-Modal Whole Slide Image Analysis](https://arxiv.org/abs/2507.14680) - MICCAI 2025 (Oral)

### IEEE World AI IoT Congress 2025

- `[x]` [Enhancing Clinical Decision Support and EHR Insights through LLMs and the Model Context Protocol: An Open-Source MCP-FHIR Framework](https://ieeexplore.ieee.org/document/11105280) - IEEE World AI IoT Congress 2025

### IEEE 4th International Conference 2025

- `[x]` [MCP-PUF: A Context-Hardened, ML-Assisted Authentication Protocol for Secure Healthcare IoT](https://ieeexplore.ieee.org/abstract/document/11467437) - 2025 IEEE 4th International Conference (IEEE) DONT REFERENCE

### 2025 International Conference (IEEE)

- `[X]` [Multi-Context Protocol (MCP) Hosts provide secure and real-time integration of AI tools](https://ieeexplore.ieee.org/document/11430968) - 2025 International Conference (IEEE)
- `[x]` [Blockchain Integration for Trust and Consent Management in MCP-Enabled FHIR Systems](https://ieeexplore.ieee.org/document/11466653) - 2025 IEEE International Conference (IEEE) DONT REFERENCE
- `[ ]` [The Privacy Protector: MCP-based LLM agent for Health Support](https://ieeexplore.ieee.org/abstract/document/11406275) - 2025 8th International Conference (IEEE)

### IEEE ICIP 2022

- `[ ]` [Robust 3d Cell Segmentation: Extending The View Of Cellpose](https://ieeexplore.ieee.org/document/9897942/) - 2022 IEEE International Conference on Image Processing (ICIP)

### IEEE TENCON 2025

- `[ ]` [Automated Nuclei Segmentation in PR-IHC Breast Cancer Images Using the Cellpose Deep Learning Model](https://ieeexplore.ieee.org/document/11375047/) - 2025 IEEE Region 10 Conference (TENCON)

### IEEE BIBM 2025

- `[ ]` [Data Efficiency and Transfer Robustness in Biomedical Image Segmentation: A Study of Redundancy and Forgetting with Cellpose](https://ieeexplore.ieee.org/document/11356754/) - 2025 IEEE International Conference on Bioinformatics and Biomedicine (BIBM)

### NeurIPS 2024

- `[ ]` [MDAgents: An Adaptive Collaboration of LLMs for Medical Decision-Making](https://arxiv.org/abs/2404.15155) - NeurIPS 2024 (Oral)

---

## Journals

### ACM

- `[ ]` [Model Context Protocol (MCP): Landscape, Security Threats, and Future Research Directions](https://dl.acm.org/doi/10.1145/3796519) - ACM Transactions on Software Engineering and Methodology

### IEEE

- `[ ]` [https://ieeexplore.ieee.org/document/11319419](https://ieeexplore.ieee.org/document/11319419) - IEEE Journal of Biomedical and Health Informatics ?
- `[x]` [A Review of Methods for Trustworthy AI in Medical Imaging: The FUTURE-AI Guidelines](https://ieeexplore.ieee.org/document/11183651) - IEEE
- `[ ]` [Agentic AI in Healthcare and Medicine: A Seven-Dimensional Taxonomy for Empirical Evaluation of LLM-Based Agents](https://ieeexplore.ieee.org/document/11329025/) - IEEE Access
- `[ ]` [Agentic AI: A Comprehensive Survey of Technologies](https://ieeexplore.ieee.org/iel8/6287639/10820123/11071266.pdf) - IEEE Access
- `[x]` [Artificial General Intelligence for Medical Imaging Analysis](https://ieeexplore.ieee.org/abstract/document/10746601) - IEEE Journal
- `[ ]` [LLM-Based Generative AI in Medicine: Analysis of Current Research Trends](https://ieeexplore.ieee.org/abstract/document/11151540) - IEEE
- `[ ]` [Redefining Elderly Care With Agentic AI: Challenges and Opportunities](https://doi.org/10.1109/OJCS.2026.3650842) - IEEE Open Journal of the Computer Society, 2026
- `[x]` [Building Trustworthy AI in Healthcare](https://doi.org/10.1109/ACCESS.2025.3648410) - IEEE Access, 2026

### Nature Methods

- `[ ]` [Cellpose: a generalist algorithm for cellular segmentation](https://www.nature.com/articles/s41592-020-01018-x) - Nature Methods, 2021

### Nature / npj

- `[ ]` [Acquisition parameters influence AI recognition of race in chest x-rays and mitigating these factors reduces underdiagnosis bias](https://www.nature.com/articles/s41467-024-52003-3) - Nature Communications

- `[ ]` [LLM-based Agentic Systems in Medicine and Healthcare](https://www.nature.com/articles/s42256-024-00944-1) - Nature Machine Intelligence
- `[x]` [The Role of Agentic AI in Healthcare: A Scoping Review](https://www.nature.com/articles/s41746-026-02517-5) - npj Digital Medicine
- `[ ]` [Benchmarking LLM-Based Agent Systems for Clinical Decision Tasks](https://www.nature.com/articles/s41746-026-02443-6) - npj Digital Medicine
- `[ ]` [AI Agent in Healthcare: Applications, Evaluations, and Future Directions](https://www.nature.com/articles/s44387-026-00076-4) - npj Artificial Intelligence
- `[ ]` [Healthcare Agent: Eliciting the Power of LLMs for Medical Consultation](https://www.nature.com/articles/s44387-025-00021-x) - npj Artificial Intelligence
- `[ ]` [Autonomous artificial intelligence prescribing a drug to prevent severe acute graft-versus-host disease in HLA-haploidentical transplants](https://www.nature.com/articles/s41467-025-62926-0) - Nature Communications
- `[ ]` [DxDirector: an agentic large language model driving the full-process clinical diagnosis](https://www.nature.com/articles/s41467-026-71928-5) - Nature Communications
- `[x]` [Large Language Models lack essential metacognition for reliable medical reasoning](https://www.nature.com/articles/s41467-024-55628-6) - Nature Communications
  - **Use in Section I.A (risks paragraph):** cite for the hallucination/overconfidence claim — models give confident answers even when correct options are absent, directly supporting "hallucination is critical in language models used for report generation"

### Radiology (RSNA)

- `[ ]` [Agentic AI in Radiology](https://pubs.rsna.org/doi/10.1148/radiol.252730) - Radiology
- `[ ]` [Agentic AI in Radiology: Evolution from LLMs to Future Clinical Integration](https://doi.org/10.1148/ryai.250651) - Radiology: Artificial Intelligence

### Springer

- `[x]` [Transforming Clinical Medicine with Multimodal AI, Agentic Systems, and MCP](https://link.springer.com/article/10.1007/s44250-026-00343-w) - Discover Health Systems
- `[x]` [Agentic AI: a comprehensive survey of architectures, applications, and future directions](https://doi.org/10.1007/s10462-025-11422-4) - Artificial Intelligence Review, Springer Nature, 2026
- `[ ]` [Systematic Review: Agentic AI in Neuroradiology](https://link.springer.com/article/10.1007/s10278-025-01839-2) - Journal of Imaging Informatics in Medicine

### Elsevier

- `[ ]` [Agentic LLM for anonymizing healthcare data with contextual awareness](https://doi.org/10.1016/j.knosys.2026.116034) - Knowledge-Based Systems, Elsevier, 2026
- `[ ]` [Review of Agentic AI in Radiology: From Current Clinical Integration to Future Innovations](https://www.sciencedirect.com/article/abs/pii/S0009926026001273) - Clinical Radiology
- `[ ]` [Evaluating LLMs and Agents in Healthcare: Key Challenges in Clinical Applications](https://mednexus.org/doi/10.1016/j.imed.2025.03.002) - Intelligent Medicine
- `[ ]` [Large Language Model Agents for Biomedicine: Methods, Evaluations, Challenges](https://www.mdpi.com/2078-2489/16/10/894) - MDPI Information

### MDPI

- `[x]` [Leveraging MCP and Corrective RAG for Scalable and Interoperable Multi-Agent Healthcare Systems](https://www.mdpi.com/2079-9292/15/4/888) - Electronics (MDPI), February 2026

### Taylor & Francis

- search at https://www.tandfonline.com/
- `[ ]` [MCP-enabled agentic AI workflow for building energy modelling: framework and use cases](https://www.tandfonline.com/doi/full/10.1080/19401493.2026.2653969) - Journal of Building Performance Simulation, Taylor & Francis, 2026
- `[ ]` [AI Agents and Agentic Systems: A Multi-Expert Analysis](https://www.tandfonline.com/doi/full/10.1080/08874417.2025.2483832) - Journal of Computer Information Systems, Taylor & Francis, 2025

### PubMed / PMC

- `[x]` [Agentic AI in Radiology: Emerging Potential and Unresolved Challenges](https://pmc.ncbi.nlm.nih.gov/articles/PMC12515039/) *(commentary)*
- `[ ]` [Agentic Systems in Radiology: Principles, Opportunities, Privacy Risks, Regulation](https://pubmed.ncbi.nlm.nih.gov/41432042/)

---

## Preprints (arXiv / not yet peer-reviewed) (IMPORTANT DONT READ MORE PREPRINTS)

### MCP

- `[x]` [A Survey of the Model Context Protocol (MCP): Standardizing Context to Enhance LLMs](https://doi.org/10.20944/preprints202504.0245.v1)
- `[ ]` [MCP-AI: Protocol-Driven Intelligence Framework for Autonomous Reasoning in Healthcare](https://arxiv.org/abs/2512.05365)
- `[ ]` [Agentic-AI Healthcare: Multilingual, Privacy-First Framework with MCP Agents](https://arxiv.org/abs/2510.02325)
- `[ ]` [EHR-MCP: Real-world Evaluation of Clinical Information Retrieval via MCP](https://arxiv.org/abs/2509.15957)
- `[ ]` [Enhancing MCP with Context-Aware Server Collaboration](https://arxiv.org/abs/2601.11595)
- `[ ]` [MCP Tool Descriptions Are Smelly: Towards Improving AI Agent Efficiency](https://arxiv.org/html/2602.14878v1)

### Agentic AI in Radiology / Medical Imaging

- `[x]` [Agentic Systems in Radiology: Design, Applications, Evaluation, and Challenges](https://arxiv.org/abs/2510.09404)
- `[ ]` [A Co-evolving Agentic AI System for Medical Imaging Analysis (TissueLab)](https://arxiv.org/html/2509.20279v1)
- `[ ]` RadFabric: Agentic AI System with Reasoning Capability for Radiology - arXiv, June 2025
- `[ ]` [RadAgents: Multimodal Agentic Reasoning for Chest X-ray Interpretation](https://arxiv.org/html/2509.20490v1)
- `[ ]` [Medical AI Consensus: A Multi-Agent Framework for Radiology Report Generation](https://arxiv.org/html/2509.17353)
- `[ ]` [How Well Can Modern LLMs Act as Agent Cores in Radiology Environments?](https://arxiv.org/abs/2412.09529)
- `[ ]` [Agentic LLM Workflows for Generating Patient-Friendly Medical Reports](https://arxiv.org/html/2408.01112v2)

### Agentic AI in Healthcare (broader)

- `[ ]` [Agentic AI in Healthcare and Medicine: A Seven-Dimensional Taxonomy](https://arxiv.org/html/2602.04813v1)
- `[ ]` [MedAgentBench: A Realistic Virtual EHR Environment to Benchmark Medical LLM Agents](https://arxiv.org/abs/2501.14654)
- `[ ]` [FHIR-AgentBench: Benchmarking LLM Agents for EHR Question Answering](https://arxiv.org/html/2509.19319v1)
- `[ ]` [MedAgentBoard: Benchmarking Multi-Agent Collaboration for Medical Tasks](https://arxiv.org/abs/2505.12371)
- `[ ]` [AI Agents in Clinical Medicine: A Systematic Review](https://www.medrxiv.org/content/10.1101/2025.08.22.25334232v1.full)

---

## Software / Frameworks

- [Project MONAI](https://doi.org/10.5281/zenodo.4323059) - MONAI Consortium, Zenodo, December 2020

---

## Books

- `[ ]` [AI Agents and Applications: With LangChain, LangGraph, and MCP](https://ieeexplore.ieee.org/document/11421620) - Manning, 2025
