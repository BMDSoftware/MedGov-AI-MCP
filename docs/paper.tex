...
% should we consider the reprot generation? is what appears in the result tab the report? or should we consider something else? because we just get voxels so...
% is the report even rich enough to be considered? and henrique changed the radlex so i need further testing now to see if its relevant enough but for now ill only mention the segementation and the result generated
% leaving out the generat report part for now
