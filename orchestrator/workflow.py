...
def main():
    ...
    class_names = ["Hand", "HeadCT", "AbdomenCT", "ChestCT", "BreastMRI", "CXR"]
    ...
    sample = dataset[0]   # sample is a dict
    image = sample["image"]
    label = sample["label"]
    ...
    #call mcp-monai tool to list models, according to the registerd monai service
    print_header("Calling MONAI: list_models")
    try:
        result = client.call_tool(
            service_name="monai",
            tool_name="list_models",
            arguments={"category": f"{class_names[label]}"}
        )
        print(f"Available MONAI models: {result.get('models', [])}")
    except Exception as e:
