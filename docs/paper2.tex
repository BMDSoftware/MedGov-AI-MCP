...
\begin{abstract}
Clinical AI integration and medical assistance remain fragmented: capable tools exist for imaging analysis, report generation, and clinical advising, but operate in isolation, requiring custom integration work that slows adoption. This paper presents MedGov-AI, an agentic system that addresses this through the Model Context Protocol (MCP) as a standardized interoperability layer. A large language model acts as an autonomous orchestrator, discovering and invoking clinical tools at runtime through MCP servers covering DICOM parsing, CT segmentation (MONAI), whole-slide image cell segmentation (Cellpose), radiology report generation via the RadLex ontology, and digital pathology (iPath). Domain-specific workflow governance is provided through skill files - plain-text protocol documents that encode clinical sequences and guard conditions without model retraining. The system supports both interactive and event-driven operation; in workspace monitoring mode, files deposited into a configured directory trigger autonomous agent execution without user input.

Three end-to-end use cases were evaluated: CT analysis with structured radiology report generation; interactive whole-slide tumor detection via iPath with cell segmentation and pathology reporting; and autonomous workspace monitoring with Cellpose segmentation and file routing. API-hosted large language models completed multi-step clinical workflows reliably within a bounded iteration budget; locally-hosted 7--8B parameter models did not, revealing a tension between privacy and reliability in clinical deployment. Skill-guided execution prevented clinically incorrect tool selection at ambiguous decision points. Six MCP servers were integrated without any agent-side connector code, supporting the argument that MCP can reduce the integration overhead currently limiting clinical AI adoption. The system is publicly available at \url{https://medgovai.bmd-software.com/}.
\end{abstract}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.5cm}p{9cm}}
    \toprule
    \textbf{Problem or Issue} & Clinical AI tools for medical imaging and pathology operate in isolation with no standard integration layer, forcing institutions to build and maintain custom connectors that slow adoption and create fragmented, unmaintainable deployments. \\
    \midrule
    \textbf{What is Already Known} & AI tools for radiology and pathology exist and are clinically deployed. Agentic AI can autonomously execute multi-step tool-calling tasks. MCP provides a standardized protocol for agent-tool integration. Skill files can guide LLM behavior without retraining. \\
    \midrule
    \textbf{What this Paper Adds} & An implemented agentic system integrating six clinical AI tools via MCP without custom connector code; empirical evidence that skills change agent behavior at clinical decision forks (0/10 vs 10/10 success); and a benchmark comparing API-hosted and local LLMs for clinical tool-calling reliability. \\
    \midrule
    \textbf{Who Benefits} & Clinical informaticians reducing integration overhead; institutions seeking auditable AI-assisted workflows; researchers developing agentic AI for healthcare. \\
    \bottomrule
  \end{tabular}
\end{table}
...
The broader contribution is a demonstration that MCP combined with a skills system for domain-specific governance provides a practical, extensible integration layer for clinical AI. Any tool wrappable in an MCP server becomes immediately accessible to the agent, and as more tools adopt MCP the integration overhead that limits adoption~\cite{collaco2026scoping} shifts from adapter engineering to tool capability itself. A working deployment of MedGov-AI is publicly accessible at \url{https://medgovai.bmd-software.com/}.
