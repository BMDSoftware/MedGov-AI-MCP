...
Clinical AI tools for medical imaging are increasingly capable but remain largely isolated from one another, requiring custom integration work that slows adoption and creates fragmented, difficult-to-maintain deployments. This paper presents MedGov-AI, an agentic system that addresses this through the Model Context Protocol (MCP) as a standardized interoperability layer. A large language model acts as an autonomous orchestrator, discovering and invoking clinical tools at runtime through MCP servers covering a broad range of tasks including DICOM parsing, computed tomography (CT, a 3D volumetric imaging modality) segmentation, radiology report generation, whole-slide image cell segmentation, and digital pathology. Domain-specific workflow governance is provided through skill files that encode clinical sequences and guard conditions without model retraining. The system supports both interactive and event-driven operation, where files ingestion automatically trigger autonomous agent execution without user input.
...
We built a custom agentic system to enforce clinical-specific behaviours: a completion contract under which the agent cannot declare success while deliverables remain unmet, and an optional per-call confirmation mode for operator oversight.
...
\textbf{Mcp-cellpose} provides cell segmentation using Cellpose~\cite{cellpose}. Version 3 exposed different models for different tasks, for example, whole-cell counting (cyto, cyto2, cyto3), nucleus detection (nuclei), or bacteria segmentation (bact), which made it initially relevant for testing the orchestrator's capacity to identify and call the correct model depending on the task requested. Version 4 replaces all of these with a single universal model, removing the need for explicit selection. Although designed to perform best on translucent imaging modalities such as phase contrast and fluorescence microscopy, tests on standard H\&E RGB images showed substantially better results, with 1{,}706 cells detected against 1{,}123 with v3 on the same ROI (Figure~\ref{fig:cellpose-comparison}), and the simplified integration made it the preferred choice. The implementation detects GPU at startup with CPU fallback, practical only for small images on CPU.
...
Five models were evaluated: three API-based (Gemini 2.0 Flash, 2.5 Flash, 3.0)~\cite{gemini2025} and two local (deepseek-r1:7b~\cite{deepseekr1}, llama3.1:8b~\cite{llama31}). The loop was capped at 10 iterations; a model that does not signal task completion within that budget fails regardless of correct individual tool selections.
...
For Use Case 3, two workspaces validated the trigger pipeline and instruction following without cross-routing. The \textbf{CELLPOSE} workspace was given a no-op prompt (\textit{``Do nothing, just a placeholder for images.''}); deposited files produced no action, confirming no bias toward running tools when the prompt does not call for it. The \textbf{ROI} workspace was configured verbatim: \textit{``To this folder there will arrive ROI images, I want you to count the number of cells for each image that arrives and I want that if the number of cells counted is above 100 to send the image to the folder in /app/workspaces/CELLPOSE.''} When a 2{,}352$\times$2{,}673~px H\&E ROI image was deposited (Figure~\ref{fig:workspace-use-case}), the watcher detected it within seconds, triggered the agent after the debounce window, and the agent autonomously ran cell segmentation, reported 1{,}706 cells, and - exceeding the threshold - moved the image to CELLPOSE as instructed. The full sequence took $\approx$30~s on a local GPU (RTX 3050), confirming end-to-end operation of the watcher, debounce trigger, tool selection, and cross-workspace routing via a plain-language rule.
